...
v1.4_1 is a world-cultivation observation build. It preserves v1.4 behavior and
adds post-hoc trace surfaces for biography, memory lineage, support chains,
project aftermath, and role labels. It must not change the world dynamics.
"""
...
NAMES = [
    "Arun", "Mira", "Kai", "Sena", "Rut", "Fara",
    "Taro", "Ish", "Noa", "Erin", "Zol", "Hana",
    "Ren", "Shion", "Yura", "Gil", "Mei", "Dan",
    "Val", "Tia", "Orn", "Kei", "Rio", "Sana",
]
...
class SocietyLab:
    def __init__(self, seed: Optional[int] = None, verbose: bool = False):
        ...
        self.actor_biography: dict[str, list[dict[str, object]]] = {}
        self.memory_lineage_trace: list[dict[str, object]] = []
        self.support_chain_trace: list[dict[str, object]] = []
        self.project_aftermath_trace: list[dict[str, object]] = []
    ...
    def _next_memory_id(self) -> str:
        memory_id = f"m{self.next_memory_index:05d}"
        self.next_memory_index += 1
        return memory_id

    def _bio(self, person_id: str, event: str, detail: dict[str, object]) -> None:
        entry = {"year": self.year, "bio_event": event, **detail}
        self.actor_biography.setdefault(person_id, []).append(entry)
    ...
    def _make_memory(
        ...
        clamped_intensity = self._clamp(intensity)
        self.memory_lineage_trace.append(
            {
                "year": self.year,
                "memory_id": memory_id,
                "parent_memory_id": "",
            ...
            for proposal in proposals:
                ...
                self._bio(
                    ...
                    "proposal_made",
                    {
                        ...
                        "problem": proposal.target_problem,
                        "fear": round(proposer.fear, 3),
                        "voice_weight": round(proposer.voice_weight, 3),
                    },
                )
    ...
    def _support_proposals(self, problem: Problem, proposals: list[Proposal]) -> None:
        ...
        for person in self.alive_people:
            ...
            self.support_chain_trace.append(
                {
                    "year": self.year,
                    "supporter_id": person.id,
                    "proposer_id": proposal.proposer_id,
                    "action": proposal.action,
                    "problem": problem.type,
                    "score": round(score, 4),
                    "support_chance": round(support_chance, 4),
                    "support_nudge": round(support_nudge, 4),
                    "trust_to_proposer": round(person.trust.get(proposal.proposer_id, 0.15), 4),
                    "proposer_voice_weight": round(proposer.voice_weight, 4),
                    "supporter_fear": round(person.fear, 4),
                }
            )
            self._bio(
                person.id,
                "proposal_supported",
                {
                    ...
                    "action": proposal.action,
                    "problem": problem.type,
                    "support_chance": round(support_chance, 3),
                    "support_nudge": round(support_nudge, 3),
                },
            )
            self._bio(
                proposal.proposer_id,
                "proposal_received_support",
                {
                    "supporter_id": person.id,
                    "action": proposal.action,
                    "problem": problem.type,
    ...
    def _execute_project(self, problem: Problem, proposal: Proposal) -> None:
        ...
        before = {
            "food_stock": self.food.stock,
            "food_capacity": self.food.capacity,
            "proposer_voice_weight": proposer.voice_weight,
            "supporter_fear_sum": sum(person.fear for person in supporters),
            "supporter_hunger_sum": sum(person.hunger for person in supporters),
            "supporter_trust_to_proposer_sum": sum(
                person.trust.get(proposer.id, 0.0) for person in supporters
            ),
        }
        for participant in [proposer] + supporters:
            self._bio(
                participant.id,
                "project_started",
                {
        ...
        aftermath = {
            ...
            "food_stock_delta": round(after["food_stock"] - before["food_stock"], 4),
            "food_capacity_delta": round(after["food_capacity"] - before["food_capacity"], 4),
            "proposer_voice_delta": round(
                after["proposer_voice_weight"] - before["proposer_voice_weight"], 4
            ),
            "supporter_fear_delta": round(
                after["supporter_fear_sum"] - before["supporter_fear_sum"], 4
            ),
            "supporter_hunger_delta": round(
                after["supporter_hunger_sum"] - before["supporter_hunger_sum"], 4
            ),
            "supporter_trust_to_proposer_delta": round(
                after["supporter_trust_to_proposer_sum"]
                - before["supporter_trust_to_proposer_sum"],
                4,
            ),
        }
        self.project_aftermath_trace.append(aftermath)
        for participant in [proposer] + supporters:
            self._bio(
                participant.id,
                event,
        ...
        for speaker in self.alive_people:
            ...
            for listener in listeners:
                ...
                self.memory_lineage_trace.append(
                    {
                        "year": self.year,
                        "memory_id": distorted.memory_id,
                        "parent_memory_id": memory.memory_id,
                ...
                self._bio(
                    ...
                    "memory_received",
                    {
                        ...
                        "event": distorted.event,
                        "intensity": round(distorted.intensity, 3),
                        "interpretation": distorted.interpretation,
                    },
                )
                self.log.record(
        ...
        for person in self.alive_people:
            ...
            self._bio(
                ...
                "behavior_copied",
                {
                    ...
                    "new_action": person.action,
                    "fear": round(person.fear, 3),
                },
            )
            self._bio(
                target.id,
    ...
    def _generation_phase(self) -> None:
        ...
        for parent in candidates:
            ...
            self._bio(
                ...
                {
                    "child_id": child.id,
                    "food_factor": round(food_factor, 3),
                    "fear_factor": round(fear_factor, 3),
                    "population_factor": round(population_factor, 3),
                },
            )
            self._bio(
                child.id,
                ...
                {
                    "parent_id": parent.id,
                    "initial_trust_parent": round(child.trust.get(parent.id, 0.0), 3),
                },
            )
            self.log.record(
                self.year,
    ...
    def summary(self) -> dict[str, object]:
        return {
            ...
            "biography_subject_count": len(self.actor_biography),
            "memory_lineage_trace_count": len(self.memory_lineage_trace),
            "support_chain_trace_count": len(self.support_chain_trace),
            "project_aftermath_trace_count": len(self.project_aftermath_trace),
        }

    def observation_summary(self) -> dict[str, object]:
        return {
            "actor_biography_subjects": len(self.actor_biography),
            "memory_lineage_events": len(self.memory_lineage_trace),
            "support_chain_events": len(self.support_chain_trace),
            "project_aftermath_events": len(self.project_aftermath_trace),
            "role_labels": self.detect_posthoc_roles(),
        }

    def actor_story(self, person_id: str) -> list[dict[str, object]]:
        return self.actor_biography.get(person_id, [])

    def detect_posthoc_roles(self) -> dict[str, list[str]]:
        role_scores: dict[str, dict[str, int]] = {}

        def add(person_id: str, role: str, amount: int = 1) -> None:
            role_scores.setdefault(person_id, {})
            role_scores[person_id][role] = role_scores[person_id].get(role, 0) + amount

        for entry in self.memory_lineage_trace:
            holder_id = str(entry["holder_id"])
            source_id = str(entry["source_id"])
            if entry["mode"] == "created":
                add(holder_id, "memory_holder")
            elif entry["mode"] == "shared":
                add(holder_id, "memory_receiver")
                add(source_id, "memory_carrier")

        for entry in self.support_chain_trace:
            add(str(entry["supporter_id"]), "supporter")
            add(str(entry["proposer_id"]), "supported_proposer")
            if float(entry["support_nudge"]) != 0.0:
                add(str(entry["supporter_id"]), "interpretation_moved_supporter")

        for entry in self.project_aftermath_trace:
            proposer_id = str(entry["proposer_id"])
            if entry["event"] == "project_success":
                add(proposer_id, "successful_proposer")
            else:
                add(proposer_id, "failed_proposer")
            if float(entry["proposer_voice_delta"]) > 0:
                add(proposer_id, "voice_gainer")

        for person_id, entries in self.actor_biography.items():
            copied_count = sum(
                1 for entry in entries if entry["bio_event"] == "behavior_was_copied"
            )
            death_memory_count = sum(
                1
                for entry in entries
                if entry["bio_event"] in {"memory_created", "memory_received"}
                and entry.get("event") == "death"
            )
            if copied_count >= 3:
                add(person_id, "behavior_model", copied_count)
            if death_memory_count >= 3:
                add(person_id, "death_memory_holder", death_memory_count)
        ...
        thresholds = {
            ...
            "memory_carrier": 3,
            "supporter": 5,
            "supported_proposer": 5,
            "interpretation_moved_supporter": 2,
            "successful_proposer": 2,
            "failed_proposer": 2,
            "voice_gainer": 2,
            "behavior_model": 3,
            "death_memory_holder": 3,
        }
        for person_id, scores in role_scores.items():
            active = [
                role
                for role, score in sorted(scores.items())
                if score >= thresholds.get(role, 1)
            ]
            if active:
                labels[person_id] = active
        return labels
